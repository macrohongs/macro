...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            responseHandler: function(res) {
                ...
                if (res.code == 0) {
                    if ($.common.isNotEmpty(table.options.sidePagination) && table.options.sidePagination == 'client') {
                    	...
                    	if(null != res.remark && "" != res.remark){
                        	$("#divRemarkRow").attr("style","display:block");
                    		$('#divRemark').html(res.remark);
                    	}
                    	if(null != res.requestDesc && "" != res.requestDesc){
                    		$('#divRequest').html(res.requestDesc);
                    	}
                    	if(null != res.responseDesc && "" != res.responseDesc){
                    		$('#divResponse').html(res.responseDesc);
                    	}
            ...
            // 导入数据
            importListExcel: function(formId) {
            	table.set();
            	var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
            	layer.open({
            		type: 1,
            		...
            		maxmin: true,
            		shade: 0.3,
            		title: '预览' + table.options.modalName + '数据',
            		content: $('#' + currentId).html(),
            		...
            		btn1: function(index, layero){
            			...
            			$.ajax({
            				...
            				success: function (result) {
            					if (result.msg == "smy"){
            						$.modal.closeAll();
                                	$("#li_01").attr("style","display:none");
                                	$("#li_02").attr("style","display:none");
                                	$("#li_03").attr("style","display:none");
                                	$("#li_04").attr("style","display:none");
                                	$("#li_05").attr("style","display:none");
                                	$("#li_06").attr("style","display:none");
                                	$("#li_07").attr("style","display:none");
                                	$("#li_08").attr("style","display:none");
                                	$("#li_09").attr("style","display:none");
                                	$("#li_10").attr("style","display:none");
                                	$("#li_11").attr("style","display:none");
                                	$("#li_12").attr("style","display:none");
                                	$("#li_13").attr("style","display:none");
                                	$("#li_21").attr("style","display:none");
                                	$("#li_24").attr("style","display:none");
                                	$("#li_25").attr("style","display:none");
                                	$("#li_26").attr("style","display:none");
            						$.each(result.data, function(key,val){
                                    	$.table.search('account-ta-form', 'bootstrap-table' + key);
                                    	$("#li_" + key).attr("style","display:block");
                    				});
            					}else{
                					if (result.code == web_status.SUCCESS) {
                						$.modal.closeAll();
        ...
        operate: {
        	...
        	// 提交数据
        	submitData: function(url, type, dataType, data, callback) {
            	var config = {
        	        url: url,
        	        type: type,
        	        dataType: dataType,
        	        ...
        	        success: function(result) {
        	        	$.each(result, function(key,val){
        	        		$('#'+key).val(val);
        	        		if(key == "taFund" && val == "0"){
                            	$.table.search('account-ta-fund-form', 'bootstrap-table');
        	        		}
        				});
        	        	var fieldSteps = $('#fieldStep').val();
        	        	if(null != fieldSteps && "" != fieldSteps){
        	        		for(var i = 1;i <= fieldSteps; i++){
            ...
            postData: function(url, data, callback) {
            	$.operate.submitData(url, "post", "json", data, callback);
            },
            // get请求传输
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.Glf_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
